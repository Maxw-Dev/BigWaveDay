...
func start_wave() -> void:
	...
	wave.reset()
	surfer.reset(Vector2(start_lead, surfer.shape.arc_length * 0.5), Vector2(spawn_speed, 0.0))
	camera.snap()
...
func _physics_process(delta: float) -> void:
	...
	if time_left <= 0.0:
		reason = "closed out"
	elif surfer.pos.x < wave.foam_u:
		reason = "caught by the foam"
	elif not surfer.forgiving and not surfer.soft_lip and surfer.pos.y > surfer.shape.arc_length:
		reason = "over the lip"
